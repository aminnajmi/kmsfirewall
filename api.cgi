#!/usr/bin/perl
# Phase 5A: JSON API with status, whitelist listing, and whitelist add.
...
sub request_action {
	my ($parameters, $error) = parse_parameters(
		$ENV{'QUERY_STRING'} || '',
		{
			action => 1,
		}
...
sub post_parameters {
	...
	return (undef, 'Invalid request body')
		if $length !~ /\A\d+\z/ || $length > 4096;

	return (undef, 'Unsupported request content type')
		if ($ENV{'CONTENT_TYPE'} || '')
			!~ /\Aapplication\/x-www-form-urlencoded(?:;|\z)/i;
	...
	while (length($body) < $length) {
		...
		return (undef, 'Unable to read request body')
...
# Load Webmin configuration and module libraries.
my $loaded = eval {
	&init_config();

	do './kmsfirewall-lib.pl'
		or die 'module library unavailable';

	do './auth-lib.pl'
		or die 'authorization library unavailable';
...
send_error(
	403,
	'FORBIDDEN',
	'KMS Firewall API access denied'
) if !api_authorized();
...
if ($action eq 'add') {
	...
	send_error(
		...
		'INVALID_ADDRESS',
		'Address is required'
	) if !exists($parameters->{'address'})
		|| $parameters->{'address'} eq '';
	...
	my $address = normalize_ipv4_or_cidr(
		$parameters->{'address'}
	);
